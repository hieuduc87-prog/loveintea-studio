...
${body.topic ? `CHỦ ĐỀ CAROUSEL: ${body.topic}` : ''}

YÊU CẦU:
- Viết TOÀN BỘ chữ bằng ${langName}. Ngắn, mạnh, đọc nhanh khi lướt.
- Chọn 1 layout DUY NHẤT dùng chung cho cả ${n} slide, trong: ${LAYOUTS.join(', ')}.
- Mỗi slide: headline ≤7 từ; sub 1 câu ngắn bổ trợ (có thể rỗng); cta chỉ ở slide cuối (vd "Mua ngay"), slide khác để rỗng; badge để rỗng trừ khi promo.
- Đánh số mạch nội dung tự nhiên (không cần ghi "slide 1/5" vào chữ).

Trả ONLY JSON: {"layout":"...","slides":[{"headline":"...","sub":"...","cta":"","badge":""}, ... đúng ${n} phần tử]}`;

    const o = await generateJSON<{ layout?: string; slides?: Array<{ headline?: string; sub?: string; cta?: string; badge?: string }> }>(prompt);
    const layout = (LAYOUTS.includes(String(o.layout)) ? String(o.layout)
      : (body.layout && LAYOUTS.includes(body.layout) ? body.layout : 'bottom-headline')) as OverlayLayout;
