...
- [session 2026-08-22] `eece43e` (11 files 345+/-51, LIVE prod ✅) **[LIT-FIX-0822A]**: 11 card kanban (10 hlt + 1 lit, 8 mới + 2 kẹt + 1 lit). Trace từ code + 1 screenshot. **8 fix hệ thống**: (A) LAYOUTS whitelist tập trung — 3 route hardcode → export LAYOUT_IDS lib/text-overlay.ts, hết silent fallback layout mới. (B) Moodboard inject image gen — hôm qua chỉ save DB, hôm nay `getMoodboardPaths()` bọc runWithBrand đọc tenant DB, append vào editProductImage.extraImagePaths (cap 2 ảnh) + hint text cho generateImage. (C) Ratio thật — saveImageToFile nhận `ratio` param, RATIO_TARGETS lookup, sharp crop TỈ LỆ THẬT user chọn (không hardcode 4:5). (D) Color Palette editor — 4 color pickers + role description. (E) Voice Traits editor — input chip editable + xoá + auto-save on blur. (F) CTA custom Content Workshop — localStorage `ech.customCta.<brandId>` MVP không cần DB migration. (G) Chữ bị cắt template PARTIAL — prompt hardening "STRICTLY NO text + safe margin 12%" giảm rủi ro không diệt 100% (gpt-image-2 không tuân prompt tuyệt đối). (H) Font Chữ trên ảnh — bọc runWithBrand trong brandFonts() để tenant DB resolve đúng. 11 kanban fixResult THÂN THIỆN format ✅/⚠️ + 🔍/🎯/⚠️. **2-LANE KNOW-HOW mới** (user lệnh): Lane 1 CODE (saas-know-how-aug-2026.md, 15 luật cho DEV), **Lane 2 CONTENT SẢN XUẤT** (content-production-know-how.md MỚI, 7 nhóm ABCDEFG cho nhân viên content + AI: hard limit AI, brand DNA setup, layout hiệu quả, prompt engineering, workflow content team, publish, sự cố). Mỗi fix phân loại đúng lane. **Luật mới**: whitelist domain centralize lib/ export; data model + UI editor là 1 hệ; per-tenant query bọc runWithBrand; localStorage MVP trước DB migration. CHRONICLE #42.
