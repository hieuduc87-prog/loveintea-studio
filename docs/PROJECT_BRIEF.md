...
- [session 2026-07-30] TÁI CẤU TRÚC ĐA-BRAND TẬN GỐC [LIT-ARCH-0729D]: nhổ 63 chỗ `|| 'loveintea'` (getBrandId bỏ default admin — thiếu brand: đọc RỖNG/ghi 400 `requireBrand`); backfill 15 kanban + 2 flows; image_jobs/publish_log/subscriptions ADD brand_id (0 dòng vô chủ); guard CI luật 4 cấm default-tenant (test 3 bước — lần đầu guard đặt SAU exit: bắt mà không chặn, đúng luật L6); `docs/multi-brand-doctrine.md` = 20 sự cố → 6 luật → checklist PR. Verified: 30/30 isolation local + prod smoke (admin no-brand: đọc rỗng, ghi 400, có brand OK). 4 kanban card 29/7 fixed [LIT-FIX-0729A].
