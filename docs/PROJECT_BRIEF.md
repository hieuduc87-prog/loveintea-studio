...
- [session 2026-08-03b] **LƯỚI VẬN HÀNH**: CI GitHub Actions (typecheck→tenant-guard→47 test đơn vị→build→boot→cách ly 30/30, đã chạy thật 2 lần liên tiếp, sẵn sàng ở 63s); `lib/logger.ts` log JSON + báo động Telegram (gộp trùng 10', trần 20 tin/giờ, che secret) nối vào scheduler/publish/thanh toán/crash toàn cục; `/api/health/live` CÔNG KHAI cho máy dò uptime ngoài (middleware exclude, rate-limit 60/phút); `deploy.sh` viết lại đúng Hetzner Docker + giữ ảnh `:previous` + health check + TỰ ĐỘNG rollback, thêm `rollback.sh` và `scripts/backup-db.sh` (sqlite .backup an toàn WAL + verify + xoay vòng, đã test cả ca file hỏng). **Cần đặt `ALERT_TELEGRAM_TOKEN`/`ALERT_TELEGRAM_CHAT_ID` trong /opt/loveintea/.env, không có thì báo động TẮT.** [LIT-OPS-0803B]
- [session 2026-08-03b] 4 lỗi thật bắt được khi làm: (1) `rateLimit` cho qua request ĐẦU mỗi cửa sổ bất kể limit → `limit:0` vẫn lọt, `remaining` ra số âm — đã vá; (2) MoMo IPN đánh `momo_payments='paid'` TRƯỚC khi fulfil, fulfil hỏng thì retry rơi vào nhánh "already processed" ⇒ **đã thu tiền, không cấp gói, tự nó không sửa được** — mới chỉ thêm báo động, LOGIC CHƯA SỬA; (3) `/api/blog/[id]`, `/api/image-library/[id]` trả **500 thay vì 404** trong ~60s đầu sau boot (migration DB-per-tenant chưa xong) — CI chờ `scheduler:"ok"` để né, gốc chưa vá; (4) Dockerfile mặc định `PORT=3200` trong khi prod map 3202 — đã sửa. [LIT-OPS-0803B]
- [session 2026-08-03b] Đo lại năng lực repo bằng số THẬT: 50 commit/6 ngày, 41k dòng TS, 135 route API, 1 tác giả. Rate limit **ĐÃ CÓ SẴN** (lib/rate-limit.ts, 30 chỗ gọi) — chỉ thiếu ở route công khai bypass middleware (casso webhook, momo IPN, autofix), nay đã bịt. Còn thiếu: ESLint chưa cấu hình (`next lint` hỏi tương tác → chưa đưa vào CI), SQLite 1 node, bus factor = 1. [LIT-OPS-0803B]
