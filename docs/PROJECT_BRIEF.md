...
- [session 2026-08-22b] `5a2e15a` (LIVE prod ✅) **[LIT-FIX-0822B]**: User audit "root hay vá?" → 5/8 fix hôm qua root thật, 3/8 vá. Fix nốt 3 nợ + GUARD chống tái phát: **#C-nợ** 3 route hardcode 1024x1536 (content/image, image/generate, plans) → add ratio param + import ratioToGptSize (hôm qua chỉ template-generate). **#F-nợ** CTA localStorage → migrate DB brand_dna.custom_ctas_json (cross-device) + auto-migrate localStorage cũ 1 lần. **#H-nợ** audit lib/ query tenant table: fix `brandColors` + `getBrandLanguage` + `getModelLook` bọc runWithBrand — 15 file backlog audit dần. **GUARD script** `scripts/guard-root-fixes.sh` 3 lớp: LAYOUTS hardcode FAIL / tenant context WARN / Gemini model hardcode FAIL — wire vào `npm run build`. Docker fix: shebang `#!/bin/sh` (Alpine không có bash) + `build:nogurad` cho Docker `RUN`. **Kết quả**: 7/8 fix ROOT + 1 vá vĩnh viễn documented (#G gpt-image-2 hard limit không nhận font — architectural 2-step overlay là scope dài). Bài học: fix root cần AUDIT SCOPE ĐẦY ĐỦ (grep TẤT CẢ callers, không dừng 1 nơi); GUARD > GIẤY (script FAIL build > luật memory); VÁ có 2 loại — chờ root (deadline) vs vĩnh viễn (documented). CHRONICLE #43.
- [session 2026-08-22a] 2 commit — docs(brief) · fix(kanban-hlt+lit-8)
