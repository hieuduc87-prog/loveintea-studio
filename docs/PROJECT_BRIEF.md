...
- [session 2026-08-21] `6ba733b` (12 files 502+/-23, **LIVE prod ✅**) **[LIT-FIX-0821A]**: 6 card kanban (2 loveintea + 4 hoa-lang-thang), description trống — trace từ code base. 1 card LẶP hôm qua (`c7174152` không xoá SP) — root cause thực sự ở FRONTEND thiếu nút UI, không phải backend. **6 fix hệ thống**: (#6) Add nút Xoá + Sửa SP + modal EditProductModal (khớp PATCH allowlist) — hôm qua chỉ fix backend DELETE, hôm nay add UI (bài học: verify CẢ 2 tầng). (#5) Ratio selector template gen — new `ratioToGptSize()` map 1:1/4:5/9:16/16:9/3:2 → gpt-image-2 size + UI 4 nút. (#2) 3 layout text mới: script-hero cursive, split-diagonal tam giác brand, editorial-caption serif magazine (dùng c.primary sẵn). (#1) Nút Sửa chữ single mode TextOverlay (setResult('') giữ base+fields). (#3) Moodboard Brand DNA — ALTER moodboard_json + route GET/POST multipart/DELETE + MoodboardSection grid sharp resize 1600 cap 24 ảnh. (#4) PARTIAL font brand cho template — gpt-image-2 KHÔNG nhận font input (hard limit), fix minh bạch: banner amber + 1-click chuyển sang Chữ lên ảnh (CustomEvent open-text-overlay). Áp cho MỌI brand. 11 card kanban fixed. Gotcha: c.brand không tồn tại (dùng c.primary); TS strict inferred any. **Luật**: backend + frontend là 1 hệ (test API OK ≠ user done, phải có UI); gpt-image-2 hard limit font documented > hack; ratio phải là param không hardcode; cột mới phải update allowlist route PATCH; CSS `${c.field}` sai key = TS strict báo lỗi build. CHRONICLE #40.
