...
- **2026-07-27 [LIT-VID-0727A]** (plan, chưa code): **MASTER PLAN Creative Hub Reel Machine** tại `docs/video-auto-reel-master-plan.md` — auto AI video line từ đề bài kanban `70dc1102` (Google Doc 5 phiếu A-E của Hà Phương: Iced Summer Reel 10s, timeline 8 block theo giây, 6 SKU, palette/font/safe-zone, guardrail cấm copy đối thủ + cấm AI vẽ pack/chữ). Kiến trúc 7 module tái dùng Video Studio v2 + fal.ai (FLUX t2i → Hailuo i2v, FAL_KEY sẵn ở quizzlee-studio), 5 phase ~6 buổi, ~$1.2-1.8/video 3 bản. **5 video mẫu + đề bài đã tải lên Hetzner** `/opt/loveintea/data/references/loveintea/iced_summer_reels/`. Chặn: cần packshot alpha 6 SKU + logo + font Sorean từ nhân viên (mục 5 plan). OpenAI billing đang hard-limit → pipeline không phụ thuộc OpenAI.
