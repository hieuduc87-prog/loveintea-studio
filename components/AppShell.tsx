...
import { ReelFactoryView }    from './ReelFactoryView';
...
type TabId =
  ...
  | 'plan_calendar'
  | 'create_studio' | 'text_overlay' | 'video_studio' | 'reel_factory' | 'inspiration' | 'blog_factory' | 'cost'
...
const TABS: { id: TabId; label: string; icon: string; group: string }[] = [
  ...
  { id: 'reel_factory',     label: 'Reel Factory',     icon: '🏭', group: 'Create' },
          ...
          {!brandsLoaded ? (
            ...
            return (
              <div key={id} className={`absolute inset-0 overflow-auto ${isActive ? '' : 'hidden'}`}>
                ...
                {id === 'reel_factory'     && <ReelFactoryView brandId={bid} />}
